...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        ...
        SLLNode<Card> tmmp=firstSorcererCards.getFirst();
        int max=0, m=0;
        while (tmp!=null){
            m=tmp.element.significance();
            if(m>max){
                max=m;
                tmmp=tmp;
            }
        ...
        int brojach=0;
        SLLNode<Card>flag=secondSorcererCards.getFirst();
        while(brojach<3){
            brojach++;
            flag=flag.succ;
        }
        secondSorcererCards.insertAfter(tmmp.element, flag);
        firstSorcererCards.delete(tmmp);
