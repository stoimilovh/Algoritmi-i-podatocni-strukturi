...
class BTree<E extends Comparable <E>> {
    ...
    public boolean daliePogolemR(BNode<E> node, E number){
        if(node==null){
            return true;
        }
        if(node.info!=null && number!=null && node.info.compareTo(number)<=0){
            return false;
        }
        return daliePogolemR(node.left, node.info)&&daliePogolemR(node.right, node.info);
    }

    public boolean daliePogolem(){
        E pocetnavrednos=null;
        return daliePogolemR(root, pocetnavrednos);
    }
...
public class ConsecutiveNumbers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // vasiot kod ovde
        boolean flag= tree.daliePogolem();
